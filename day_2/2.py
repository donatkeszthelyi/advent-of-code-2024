...
def adj_diff_check(row):
    ...
    for i in range(len(row) - 1): #Check if any adjacent pair has a difference between 1 and 3 (inclusive)
...
def strict_monotonity_check(row):
    row = row.dropna() #Drop NaN values
    increasing = list(row) == sorted(row) #Check if strictly increasing
    decreasing = list(row) == sorted(row, reverse=True) #Check if strictly decreasing
...
def check_with_one_dropped(row):
    ...
    for i in range(len(row)):
        ...
        if strict_monotonity_check(row_without_i) and adj_diff_check(row_without_i): #Check if the row without the i-th value satisfies both conditions
            return True
    return False
